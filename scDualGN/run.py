...
class scDualGN():
    def __init__(self, adata, n_enc_1=512, n_enc_2=128, n_z=32, batch_size=1024,
                 device='cuda:0', verbosity=False, lr=0.01):
        ...
        lr: float, default=0.01
            learning rate, for large dataset (n>5e5), lr = 0.0001
        ...
        if lr is None:
            self.lr = 0.01 if self.n_cell < 5e5 else 0.0001
        else:
            self.lr = lr
    ...
    def pretrain(self, alpha=0.01, beta=1, gamma=4): 
        """
        ...
        batch_size =  512 if self.n_cell <= 1e4 else 1024*2 # self.batch_size 
    ...
    def cluster(self, init_algorithm='kmeans', n_cluster=10,
                resolution=0.3, n_neighbors=20, eta=1, nu=0.01, n_epochs=64,
                evalution=False):
        """
        ...
        scheduler = CosineAnnealingWarmRestarts(optimizer_model, T_0=5, T_mult=3)
    ...
    def get_cluster(self, is_save=False, path_dir=None, evaluation=False):
        """
        get predicted cluster label and z vectors, 
        and save results into h5ad file, as well as metrics such as ARI and NMI into .csv file if evalution.
        ...
        evaluation: bool
            whether or not calculating metrics

        """
        
        eval_batch_size = min(self.n_cell, 1024 * 8)
        ...
        y_pred_label = q_final.data.cpu().numpy().argmax(1)
        z_vec = np.concatenate(z_final)
        
        self.adata.obs['celltype_scDualGN'] = y_pred_label
        self.adata.obs['celltype_scDualGN'] = self.adata.obs['celltype_scDualGN'].astype('category')
        self.adata.obsm['X_z_scDualGN'] = z_vec
        
        if is_save:
            self.adata.write_h5ad(os.path.join(path_dir, 'scDualGN_clustering.h5ad'))
        
        if evaluation:
            assert 'celltype' in self.adata.obs, f'adata does not have celltype attribute'
            adata_y = np.array(self.adata.obs['celltype'], dtype=int)  # true label
            
            q_final_acc, q_final_nmi, q_final_ari, q_final_hs, q_final_cs, q_final_purity = evals(adata_y, y_pred_label, self.verbosity)
            ...
            if is_save:
                ...
                pd.DataFrame.from_dict(res).to_csv(os.path.join(path_dir, 'scDualGN_clustering_metrics.csv'), index=False)
            
    def batch_correct(self, is_save=False):
        assert 'X_z_scDualGN' in self.adata.obs, f'adata does not have embedded z vectors by scDualGN'
        assert 'batchlb' in self.adata.obs, f'adata does not have batch labels'
        sce.pp.harmony_integrate(self.adata, key='batchlb', basis='X_z_scDualGN', adjusted_basis='X_z_harmony')
        
        self.scdualgn.eval()
        h_d = self.scdualgn.decoder(self.adata.obsm['X_z_harmony'])
        x_bar = self.scdualgn.x_bar_layer(h_d)
        self.adata.obsm['X_z_harmony_decoder'] = x_bar
    ...
    def save_model(self, path_dir):
        torch.save(self.scdualgn.state_dict(), os.path.join(path_dir, 'scDualGN_model.pt'))
