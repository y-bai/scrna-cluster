...
class scDualGN():
    ...
    def pretrain(self, alpha=0.05, beta=20, gamma=4):
        ...
        # optimizer = Adam(self.dual_vae.parameters(), lr=self.lr, weight_decay=5e-4)
        
        # scheduler = CosineAnnealingLR(optimizer, T_max=10, eta_min=0)
        scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=1) # 10, 1
        # scheduler = StepLR(optimizer, step_size=5, gamma=0.1)

        n_epochs =  100 if self.n_cell < 2e4 else 50
        # n_epochs = 128 if self.n_cell < 2e4 else 64
        self.dual_vae.train()
    ...
    def cluster(self, init_algorithm='kmeans', n_cluster=10,
        ...
        # optimizer_model = AdamW(self.scdualgn.parameters(), lr=self.lr, weight_decay=5e-4) # 0.9, 5e-4
        # optimizer_centloss = AdamW(center_loss.parameters(), lr=1e-6)
        
        # optimizer_model = Adam(self.scdualgn.parameters(), lr=self.lr, weight_decay=5e-4)
        # optimizer_centloss = Adam(center_loss.parameters(), lr=1e-6)
        
        # scheduler = CosineAnnealingLR(optimizer_model, T_max=10, eta_min=0)
        scheduler = CosineAnnealingWarmRestarts(optimizer_model, T_0=5, T_mult=3) # 5, 3
